...
Reads the version from gradle.properties, extracts release notes from CHANGELOG.md,
runs pre-flight quality checks (spotlessCheck, test, build), validates repository
...
DESCRIPTION = (
    "Create and publish a GitHub release from the current repository state. "
    "Validates preconditions and runs quality checks (spotlessCheck, test, build), "
    "reads version from gradle.properties, extracts release notes from CHANGELOG.md, "
...
def _read_version(gradle_properties: Path) -> str:
    m = re.search(r"^version\s*=\s*(.+)$", _read_file(gradle_properties), re.MULTILINE)
    if not m:
        raise SystemExit(f"[ERROR] Could not find version=... in {gradle_properties}")
    return m.group(1).strip()


def _set_version(gradle_properties: Path, new_version: str, dry_run: bool) -> None:
    new_text = re.sub(
        r"^version\s*=.*$",
        f"version={new_version}",
        _read_file(gradle_properties),
        flags=re.MULTILINE,
    )
    print(f"  [edit] {gradle_properties.name}: version = {new_version}")
    if not dry_run:
        _write_file(gradle_properties, new_text)
...
def _extract_changelog_notes(changelog: Path, release_ver: str) -> str:
    ...
    # Match the entire heading line: ## v1.1.1 or ## v1.1.1-pre or ## v1.1.1 [released: ...]
...
def _validate_preconditions(release_ver: str, needs_finalize: bool, branch: str) -> int:
    # 1. Check working tree
    dirty_files = _get_dirty_files()
    allowed = {"gradle.properties", "CHANGELOG.md"} if needs_finalize else set()
    ...
    if branch not in ("main", "master"):
        print(f"[ERROR] Releases must be cut from mainline branch ('main' or 'master', currently on '{branch}')")
        return 1
    ...
    try:
        _run_capture(["git", "fetch", "origin", branch, "--tags"])
...
def _get_gradlew() -> Path:
    ...
    gradlew_path = REPO_ROOT / gradlew_name
    if not gradlew_path.exists():
        raise SystemExit(f"[ERROR] Gradle wrapper '{gradlew_name}' not found in {REPO_ROOT}")
    return gradlew_path


def _run_quality_checks() -> int:
    gradlew = str(_get_gradlew())
    print("--- Pre-flight Quality Checks ---")
    checks = [
        ("Spotless formatting (spotlessCheck)", [gradlew, "spotlessCheck"]),
        ("Test suite (test)", [gradlew, "test"]),
        ("Full build (build)", [gradlew, "build"]),
    ]
...
def release(dry_run: bool, skip_checks: bool = False) -> int:
    ...
    current_ver = _read_version(gradle_properties)
    ...
    precondition_code = _validate_preconditions(release_ver, needs_finalize, branch)
    ...
    if needs_finalize:
        ...
        _set_version(gradle_properties, release_ver, dry_run)
        if _run(["git", "add", "CHANGELOG.md", "gradle.properties"], dry_run) != 0:
            return 2
    ...
    if _run(["git", "tag", "-f", f"v{release_ver}"], dry_run) != 0:
        return 2
    if _run(["git", "push", "origin", branch], dry_run) != 0:
        return 2
    ...
    _set_version(gradle_properties, next_dev_ver, dry_run)
    if _run(["git", "add", "CHANGELOG.md", "gradle.properties"], dry_run) != 0:
        return 2
...
def main() -> int:
    ...
    parser.add_argument("--skip-checks", action="store_true", help="Skip pre-flight quality checks (spotlessCheck, test, build)")
